...
class TestRegistry:
    def test_builtin_sources(self):
        # `available_sources()` may also list plugin sources (entry-point
        # group `testql.sources`), so assert containment, not equality.
        assert (_BUILTIN_SOURCE_NAMES | _CONFIG_SOURCE_ALIASES) <= set(available_sources())
...
# ── SQL ────────────────────────────────────────────────────────────────────
# SqlSource lives in `packages/sql2testql` (resolved lazily through the
# `testql.sources` entry point); its tests moved there too.


# ── Proto ──────────────────────────────────────────────────────────────────
# ProtoSource lives in `packages/proto2testql` (resolved lazily through the
# `testql.sources` entry point); its tests moved there too.


# ── GraphQL ────────────────────────────────────────────────────────────────
# GraphQLSource lives in `packages/graphql2testql` (resolved lazily through
# the `testql.sources` entry point); its tests moved there too.
